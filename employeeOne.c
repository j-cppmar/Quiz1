...
PtrToEmployee searchEmployeeByPhoneNumber(PtrToConstEmployee ptr, int tableSize, char *givenPhone)
{
    const PtrToConstEmployee endPtr = ptr + tableSize;
    for (; ptr < endPtr; ptr++)
    {
        if (strcmp(ptr->phone, givenPhone) == 0)
...
PtrToEmployee searchEmployeeBySalary(PtrToConstEmployee ptr, int tableSize, double givenSalary)
{
    const PtrToConstEmployee endPtr = ptr + tableSize;
    for (; ptr < endPtr; ptr++)
    {
        if (ptr->salary  == givenSalary)
            return (PtrToEmployee)ptr;
    }
    return NULL;
}